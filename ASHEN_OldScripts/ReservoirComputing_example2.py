...
if __name__ == '__main__':
    ...
    Q = 6  # 输入变量的维数（对于动态系统，输入变量是三维坐标，所以是3）

    # 对于器件实现而已，我们还需要定义工作区域
    voltage_range = (0,1)  # 器件工作在0V-5V之间
    ...
    for i in range(1,num_train):
        I_normalized = np.array([np.hstack((reformed_train[i],reformed_train[i-1]))])  # GenReservoirState函数的输入必须是二维数组
        print(I_normalized)
        J_train.append(RC_reservoir.GenReservoirState(I_normalized,M)[0])  # 同时输出也是二维数组，所以取第一项就好
    J_train = np.array(J_train)
    print(J_train.shape)
    #print(J_train)
    #print(train_output[1:5000].shape)

    Wout, S = RC_readout.LASSO(J_train,train_output[1:5000])  # 利用LASSO回归进行输出权重优化

    network_train = np.dot(J_train,np.transpose(Wout))+S  # 神经网络对训练集的输出结果
    print(network_train.shape)

    # 预测阶段
    initial_input = np.hstack((train_output[len(train_output)-1],train_output[len(train_output)-2]))  # 初始输入是训练集的最后两个点
    ...
    for i in range(num_predict):
        ...
        if i == 0:
            #print(K)
            I_normalized = np.array([np.hstack((K[0],train_output[len(train_output)-1]))])  # 此时刻的输出即为下一时刻的输入
        else:
            I_normalized = np.array([np.hstack((network_predict[i],network_predict[i-1]))])
    ...
    vi.Analyzing_3Dsystem(dynamic_system[1000:16000],network_train,network_predict)