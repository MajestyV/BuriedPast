...
import numpy as np
from ReservoirComputing import CreatingWeightMatrix, DynamicalSystems
from ReservoirComputing import ActivationFunction
from ReservoirComputing import WeightOptimization
...
##################################################################
class EchoStateNetwork_Device:
    """利用这类函数，我们可以轻松调用回声状态网络算法模型"""
    def __init__(self):
        self.name = EchoStateNetwork_Device
    ...
    def ESN_for_DynamicSystems(self,training_input,training_output,ReservoirDimension,predicting_step,**kwargs):
        # 器件的性能参数
        device_param = kwargs['device_param'] if 'device_param' in kwargs else [2.036589219,5.65E-03,3.13E+01,1.0]
        ...
        for i in range(training_step):
            ...
            #test_arg = u@W_in
            #print(test_arg[0][1])
            unactivate_states = s_in*(u@W_in)+rho*(x0@W_reservoir)  # 经过这一步运算之后的变量为mat
            # print(unactivate_states)
            w_states_range.append([unactivate_states.min(),unactivate_states.max()])  # 训练部分的未激活水库态范围
            x = (1.0-a)*x0+AF.Nonlinear_IV(unactivate_states.getA(),device_param)  # 要进行下一步运算则需要换成array，即getA()
            # print(x)
            x_train.append(x[0])  # 将矩阵转换为数组后取第一个元素（即第一行）以降维
            v_train.append(np.hstack((x[0], u_train[i])))
        ...
        for i in range(predicting_step):
            unactivate_states_p = s_in*(u_i@W_in)+rho*(x_i@W_reservoir)
            w_states_range.append([unactivate_states_p.min(),unactivate_states_p.max()])  # 预测部分的未激活水库态范围
            x_p = (1.0-a)*x_i+AF.Nonlinear_IV(unactivate_states_p.getA(),device_param)
        ...
        return y_network_train,y_network_predict,W_in,W_reservoir,w_states_range
