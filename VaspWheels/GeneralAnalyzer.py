...
class functions:
    ...
    ##############################################################################################################
    # 数据拟合相关模块
    # 此函数可以对输入的多项式系数进行展开，还原多项式回归的结果，应注意：自变量x可以是浮点数或者是一维数组，多项式回归的系数列表需要是一维数组
    def Polynomial(self,x,coefficient):
        degree = len(coefficient)                                # 多项式回归的阶数
        y_list = [coefficient[i]*(x**i) for i in range(degree)]  # 根据阶数，计算每一阶对函数总值的贡献
        y_mat = np.array(y_list)                                 # 将列表转换为二维数组，即矩阵
        y = y_mat.sum(axis=0)                                    # 进行按列求和，即按列将整个矩阵求和成一行矩阵，结果为一维数组
        return y
